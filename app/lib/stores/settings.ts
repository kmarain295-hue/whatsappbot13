...
// Local type stubs — ControlPanel UI was permanently removed.
...
interface UserTabConfig extends TabVisibilityConfig {
  window: 'user';
}
...
const getInitialTabConfiguration = (): TabWindowConfig => {
  ...
  try {
    ...
    /*
     * Merge in any new tabs from DEFAULT_TAB_CONFIG that aren't in the saved config
     * (so users with existing localStorage see newly-added tabs automatically)
     */
    const savedTabIds = new Set(savedTabs.map((t: UserTabConfig) => t.id));
    const newTabs = defaultConfig.userTabs.filter((t: UserTabConfig) => !savedTabIds.has(t.id));

    return {
      userTabs: [...savedTabs, ...newTabs].sort((a, b) => a.order - b.order),
    };
  } catch (error) {
